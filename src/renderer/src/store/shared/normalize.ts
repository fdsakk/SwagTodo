...
export function normalizeStoredTask(raw: unknown): Task {
  ...
  return {
    ...
    completedAt: completed
      ...
      : undefined,
    archivedAt:
      typeof task.archivedAt === "string" ? task.archivedAt : undefined,
    ...
    subTasks: Array.isArray(task.subTasks)
      ? task.subTasks
          ...
          .filter(
            (subTask): subTask is Task["subTasks"][number] => subTask !== null
          )
      : []
  }
}
...
export function normalizeTaskPatch(
  ...
  if ("priority" in updates && updates.priority !== task.priority)
...
export function normalizeProjectPatch(
  ...
  if ("color" in updates && updates.color !== current.color)
