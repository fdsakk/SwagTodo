...
export const findIndexById = <T extends { id: string }>(items: readonly T[], id: string): number =>
...
export const getTimeRangeError = (startAt: string, endAt: string): string | null => {
  const startMs = Date.parse(startAt)
  const endMs = Date.parse(endAt)
  if (!Number.isFinite(startMs) || !Number.isFinite(endMs)) return 'Invalid start/end time'
