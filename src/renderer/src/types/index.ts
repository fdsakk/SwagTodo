...
export type AppState = Omit<
  SharedAppState,
  "pkSettings" | "appearance" | "medications"
> & {
...
export function normalizeAppearance(raw: unknown): AppearanceSettings {
  ...
  const themeId =
    ...
    THEME_PRESETS.some((p) => p.id === obj.themeId)
  ...
  if (obj.customTokensByTheme && typeof obj.customTokensByTheme === "object") {
    ...
    for (const preset of THEME_PRESETS) {
      ...
      if (Object.keys(themeTokens).length > 0)
        customTokensByTheme[preset.id] = themeTokens
    }
  }
  ...
  if (
    ...
    if (Object.keys(legacyTokens).length > 0)
      customTokensByTheme[themeId] = legacyTokens
  }
...
export function getResolvedTokens(settings: AppearanceSettings): ThemeTokens {
  const preset =
    THEME_PRESETS.find((p) => p.id === settings.themeId) ?? THEME_PRESETS[0]
